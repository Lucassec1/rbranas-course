// Definir uma família de algoritmos, encapsular cada uma delas e torná-las INTERCAMBIEÁVEIS
...
test("Deve criar um estacionamento vazio", function () {
    const parkingLot = new ParkingLot("shopping", 500, new BeachPriceCalculator());
    expect(parkingLot.getEmptySpaces()).toBe(500);
});

test("Deve entrar um carro", function () {
    const parkingLot = new ParkingLot("shopping", 500, new BeachPriceCalculator());
    parkingLot.checkin("AAA-9999", new Date("2023-15-02T10:00:00"));
...
test("Deve sair um carro", function () {
    const parkingLot = new ParkingLot("shopping", 500, new BeachPriceCalculator());
    parkingLot.checkin("AAA-9999", new Date("2023-15-02T10:00:00"));
    const price = parkingLot.checkout("AAA-9999", new Date("2023-15-02T15:00:00"));
    expect(parkingLot.getEmptySpaces()).toBe(500);
});
...
test("Deve calcular o valor que deve ser pago na praia, 20 reais por tempo ilimitado", function () {
    ...
    parkingLot.checkin("AAA-9999", new Date("2023-01-01T10:00:00"));
	const price = parkingLot.checkout("AAA-9999", new Date("2023-01-01T15:00:00"));
